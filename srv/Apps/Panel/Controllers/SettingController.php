...
class SettingController extends ControllerBase
{
    public function indexAction()
    {
        ...
        if ($this->request->isPost()) {
            $admin = $this->admin;
            $admin_data = [
                'name' => $this->request->getPost('name', array('string')),
                /*  'app_uid' => $this->request->getPost('app_uid', array('int'))*/
            ];
            ...
            if ($tag) {
                if (!Admins::updateOne($admin_data, ['id' => $admin['id']])) {
                    ...
                    $this->session->set('admin', Admins::findOne('id=' . $admin['id']));
                }
            }
            // $admin->save($data);
        ...
        if ($this->admin['app_uid'] != '') {
            $users = UserInfo::findList(["user_id in (" . $this->admin["app_uid"] . ')', 'columns' => 'username,user_id as id,avatar,sex,birthday']);
    ...
    public function securityPasswordAction()
    {
        if ($this->request->isPost()) {
            $original = $this->request->getPost('original', array('string'));
            $original = sha1($original);
            $admin = Admins::findOne(['id = ' . $this->admin['id']]);
            if ( !empty($admin['security_password']) && $original != $admin['security_password']) {
                $this->flash->error("原始密码有误");
                return false;
            }
            ...
            $this->admin['security_password'] = $password;
            $this->session->set('admin',$this->admin);
            $this->flash->success("安全密码修改成功");
        } else {
            $this->flash->notice("安全密码为执行敏感操作时，需要的二次验证密码，修改后请妥善保存");
        }
    }
