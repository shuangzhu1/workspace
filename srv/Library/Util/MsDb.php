...
class MsDb extends Plugin
{
    ...
    public function __construct()
    {
        ...
        try {
            $dsn = $config->pdoType . ':dbname=' . $config->dbname . ';host=' . $config->host . ';charset=utf8';
            self::$db = new \PDO($config->pdoType . ':dbname=' . $config->dbname . ';host=' . $config->host . ';charset=utf8', $config->username, $config->password);
    ...
    public static function insert($table, $data, $ignore = false)
    {
        ...
        if (!$sth->execute()) {
            throw new \PDOException(json_encode($sth->errorInfo()));
        }
