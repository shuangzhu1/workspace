...
class RSAUtils
{
    ...
    {
        $pi_key = openssl_pkey_get_private(file_get_contents(ROOT . '/Config/wyplus_key/seller_rsa_private_key.pem'));//这个函数可用来判断私钥是否是可用的，可用返回资源id Resource id
        $encrypted = "";
    ...
    {
        $pu_key = openssl_pkey_get_public(file_get_contents(ROOT . '/Config/wyplus_key/wy_rsa_public_key.pem'));//这个函数可用来判断公钥是否是可用的，可用返回资源id Resource id
        $decrypted = "";
