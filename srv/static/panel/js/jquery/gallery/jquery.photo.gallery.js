...
//图片查看器
    ...
    $.extend({
        ...
        openPhotoGallery: function (obj) {
            ...
            var $img = $(obj),
                imgUrl = $img[0].getAttribute('data-src');
            if (!imgUrl) return;

            //

            var original_img = (imgUrl.split('_s_')[1]).split('x');
            var original_width = original_img[0];
            var original_height = (original_img[1]).split('.')[0];


            //HTML5提供了一个新属性naturalWidth/naturalHeight可以直接获取图片的原始宽高
            var img = $img[0],
                imgHeight = original_height,//img.naturalHeight,
            ...
            $gallerys.find(".gallery-pic").each(function (i, elem) {
                ...
                var url = this.getAttribute('data-src'),
                    img = $(this)[0],
                    nH = original_height,//img.naturalHeight,
                    nW = original_width,//img.naturalWidth,
                    ratio = nW / nH,
                    w = nW,
                    h = nH;
                if (url == imgUrl) {
            ...
            $("<iframe></iframe").appendTo("body")
                .attr("id", "J_pg")
                .attr("src", "/static/panel/js/jquery/gallery/gallery.html")
                .css({
                    position: "absolute",
                    left: (document.body.clientWidth - winWidth) / 2,
                    top: (document.body.clientHeight - winHeight) / 2,
                    height: /*(document.body.clientHeight), //*/winHeight,
                    'min-width': '800px',
        ...
        initGallery: function () {
            ...
            $(".closeWin").click(function () {
                ...
                var _jg = _parent.document.getElementById("J_pg");
