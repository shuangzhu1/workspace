...
define(function (require, exports) {
    ...
    exports.checkUser = function (btn) {
        $(" .listData").on('click', btn, function (e) {
            ...
            $(this).confirm("你确定审核通过吗?", {
                ok: function () {
                    base.requestApi('/srv/api/auth/checkUser', {data: data}, function (res) {
                        if (res.result == 1) {
        ...
        $(".checkAllSelected").on('click', function (e) {
            ...
            $(this).confirm("你确定选中的项审核通过吗?", {
                ok: function () {
                    base.requestApi('/api/auth/checkUser', {data: data}, function (res) {
                        if (res.result == 1) {
                            base.showTip('ok', '操作成功！', 3000,function(){
                                window.location.reload();
                            });
    ...
    exports.failCheck = function () {
        ...
        $("#checkModal #sureBtn").on('click', function () {
            ...
            base.requestApi('/api/auth/checkFail', {id: $("#apply_id").val(), reason: reason}, function (res) {
                if (res.result == 1) {
