...
define(function (require, exports) {
    ...
    exports.edit = function () {
        ...
        $(".editBtn").on('click', function () {
            ...
            modal.find('.modal-title').html("分类编辑");
            modal.modal('show');
        });
        //添加分类
        $(".btnAdd").on('click', function () {
            ...
            modal.find("#cat_id").val(0);
            modal.find("#thumb").val('');
            modal.find(".preview-thumb").attr('src', '');
            modal.find(".error-widget").hide();
            modal.find(".success-widget").hide();
            modal.find('.modal-title').html("添加分类");
        ...
        $(".lockBtn").on('click', function () {
            ...
            $(this).confirm("确定要禁用吗?", {
                ok: function () {
                    base.requestApi('/api/music/lockCat', {
                        cat_id: cat_id
                    }, function (res) {
                        if (res.result == 1) {
        ...
        $(".unLockBtn").on('click', function () {
            ...
            $(this).confirm("确定要解除禁用吗?", {
                ok: function () {
                    base.requestApi('/api/music/unLockCat', {
                        cat_id: cat_id
                    }, function (res) {
                        if (res.result == 1) {
                            base.showTip('ok', res.data, 1000, function () {
        ...
        modal.find("#sureBtn").on('click', function () {
            var cat_id = modal.find("#cat_id").val();
            ...
            if (cat_id > 0) {
                /*   $(this).confirm("确定要修改吗?", {
                 ok: function () {*/
                base.requestApi('/api/music/editCat', {
                    cat_id: cat_id,
                    name: name,
                    sort_num: sort_num,
                    enable: enable,
